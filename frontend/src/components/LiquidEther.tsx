...
const defaultColors = ['#00d1ff', '#1152d4', '#b0e8f8'];
...
export default function LiquidEther({
    ...
    useEffect(() => {
        ...
        class ExternalForce extends ShaderPass {
            ...
            update(...args: any[]) {
                const props = args[0] || {};
                const forceX = (Mouse.diff.x / 2) * (props.mouse_force || 0); const forceY = (Mouse.diff.y / 2) * (props.mouse_force || 0);
        ...
        class Viscous extends ShaderPass {
            ...
            update(...args: any[]) {
                ...
                if (!this.uniforms) return; let fbo_in: any, fbo_out: any;
                if (typeof v === 'number') this.uniforms.v.value = v;
                const iter = iterations ?? 0;
                for (let i = 0; i < iter; i++) { if (i % 2 === 0) { fbo_in = this.props.output0; fbo_out = this.props.output1; } else { fbo_in = this.props.output1; fbo_out = this.props.output0; } this.uniforms.velocity_new.value = fbo_in.texture; this.props.output = fbo_out; if (typeof d === 'number') this.uniforms.dt.value = d; super.update(); }
                return fbo_out;
            }
        }
        ...
        class Poisson extends ShaderPass {
            ...
            update(...args: any[]) {
                ...
                for (let i = 0; i < iter; i++) { if (i % 2 === 0) { p_in = this.props.output0; p_out = this.props.output1; } else { p_in = this.props.output1; p_out = this.props.output0; } if (this.uniforms) this.uniforms.pressure.value = p_in.texture; this.props.output = p_out; super.update(); }
                return p_out;
            }
        }
        ...
        // Force a resize after the browser has had a chance to paint the container
        ...
        const io = new IntersectionObserver(entries => {
            const entry = entries[0]; const isVisible = entry.isIntersecting && entry.intersectionRatio > 0; isVisibleRef.current = isVisible;
            if (!webglRef.current) return; if (isVisible && !document.hidden) webglRef.current.start(); else webglRef.current.pause();
